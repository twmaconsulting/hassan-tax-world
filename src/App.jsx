...
/* ΓöÇΓöÇΓöÇ API helpers ΓÇö talks to local Express server ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇΓöÇ PDF text extraction (client side, for search index) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇΓöÇ LocalStorage for metadata & entries ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
async function callClaude(messages, system="") {
  try {
    ...
    if(e.message==="Failed to fetch") throw new Error("Server unreachable ΓÇö please refresh and try again.");
    throw e;
  }
}

/* ΓöÇΓöÇΓöÇ Global CSS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
const G = `
...
/* ΓöÇΓöÇΓöÇ Data Model ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
const TAX_CATS = [
...
// All doc types ΓÇö some only shown for specific categories
const DOC_TYPES = [
  { id:"laws",         label:"Laws",                    icon:"ΓÜû",  cats:null },
  { id:"cabinet",      label:"Cabinet Decisions",       icon:"≡ƒÅ¢",  cats:["ct","vat"] },
  { id:"ministerial",  label:"Ministerial Decisions",   icon:"≡ƒô£",  cats:["ct","vat"] },
  { id:"fta",          label:"FTA Decisions",           icon:"≡ƒÅó",  cats:["ct","vat"] },
  { id:"guidelines",   label:"Guidelines",              icon:"≡ƒôÿ", cats:null },
  { id:"pclars",       label:"Public Clarifications",   icon:"≡ƒôï", cats:null },
  { id:"bulletins",    label:"Bulletins",               icon:"≡ƒô░", cats:null },
  { id:"procedures",   label:"General Procedures",      icon:"≡ƒôæ", cats:null },
  { id:"hmnotes",      label:"HM Notes",                icon:"Γ£Å",  cats:null },
  { id:"usermanual",   label:"User Manual",             icon:"≡ƒôû", cats:["vat"] },
  { id:"vatfiling",    label:"VAT Filing",              icon:"≡ƒôè", cats:["vat"] },
  { id:"internet",     label:"Material from Internet",  icon:"≡ƒîÉ", cats:["vat"] },
  { id:"penalties",    label:"Penalties",               icon:"ΓÜá∩╕Å", cats:["vat","ct"] },
  { id:"taxprocedures", label:"Tax Procedures",          icon:"≡ƒôï", cats:["vat","ct"] },
  { id:"foodforthought", label:"Food for Thought",  icon:"≡ƒºá", cats:null },
  { id:"hmexam",       label:"HM Exam Material",        icon:"≡ƒÄô", cats:null },
];
...
/* ΓöÇΓöÇΓöÇ Highlight matching text ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function HL({text, q}){
...
/* ΓöÇΓöÇΓöÇ Tiny UI ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇΓöÇ Modal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(10,20,40,0.55)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(2px)"}}>
    <div style={{background:"#fff",borderRadius:"var(--radius-lg)",boxShadow:"0 8px 40px rgba(0,0,0,0.18)",width:"100%",maxWidth:wide?780:580,maxHeight:"90vh",overflow:"auto",display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"1rem 1.5rem",borderBottom:"1.5px solid var(--border)",flexShrink:0}}>
        ...
        <button onClick={onClose} style={{background:"var(--surface2)",border:"none",width:28,height:28,borderRadius:"50%",fontSize:17,color:"var(--text2)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>├ù</button>
      </div>
...
/* ΓöÇΓöÇΓöÇ Status Badge ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇΓöÇ Reusable File Upload Zone ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  return(<div>
    <div
      ...
      {uploading
        ...
        :<><p style={{fontSize:20,marginBottom:4}}>≡ƒôÄ</p><p style={{fontSize:12,fontWeight:600,color}}>{desc}</p><p style={{fontSize:11,color,opacity:0.7,marginTop:2}}>Click or drag & drop ┬╖ PDF, images supported</p></>}
    </div>
...
/* ΓöÇΓöÇΓöÇ Entry Form ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function EntryForm({catId,docTypeId,initial,onSave,onClose}){
  ...
  async function handleAttach(fileList){
    ...
    try{
      ...
      setUploadMsg(`Γ£ô ${saved.name} attached${pages>0?` ┬╖ ${pages} pages extracted`:""}`);
  ...
  return(<div>
    ...
    <div style={{background:cat.color+"12",border:`1px solid ${cat.color}30`,borderRadius:"var(--radius-sm)",padding:"8px 14px",marginBottom:16,display:"flex",gap:8,alignItems:"center"}}>
      ...
      <span style={{fontSize:12,color:"var(--text2)"}}>ΓåÆ {dt.label}</span>
    </div>
    ...
    {/* ΓöÇΓöÇ PDF Attachment ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {f.attachedFile?(
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"#ECFDF5",border:"1.5px solid #6EE7B7",borderRadius:"var(--radius-sm)"}}>
          ...
          <div style={{display:"flex",gap:6,flexShrink:0}}>
            <a href={fileViewUrl(f.attachedFile.name)} target="_blank" rel="noreferrer" style={{padding:"4px 10px",fontSize:12,background:"#fff",border:"1px solid #6EE7B7",borderRadius:"var(--radius-sm)",color:"#064E3B",textDecoration:"none",cursor:"pointer"}}>≡ƒæü View</a>
        ...
        <div
          ...
          {uploading?(
            ...
            <>
              <p style={{fontSize:22,marginBottom:4}}>≡ƒôÄ</p>
    ...
    {isFoodForThought&&(
      <div style={{background:"#FFF7ED",border:"1.5px solid #FED7AA",borderRadius:8,padding:"14px 16px",marginBottom:12}}>
        <p style={{fontSize:11,fontWeight:800,color:"#C2410C",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10}}>≡ƒºá Food for Thought Fields</p>
        ...
        <Field label="Law References"><textarea style={{...TA,minHeight:60,background:"#fff"}} value={f.articleNo||""} onChange={e=>s("articleNo",e.target.value)} placeholder="e.g. Article 55, Federal Decree-Law No. 8/2017 ΓÇö Pre-registration input tax..."/></Field>
    ...
    {/* ΓöÇΓöÇ Notes, Flashcards, Summary, Exam ΓÇö each with text + optional PDF ΓöÇΓöÇ */}
    {[
      { textKey:"notes",     fileKey:"notesFile",   label:"Notes",          icon:"Γ£Å",  color:"#0EA5E9", bg:"#F0F9FF", dashed:"#7DD3FC", placeholder:"Write your personal notes, observations, exam tips..." },
      { textKey:"flashcards",fileKey:"flashFile",   label:"Flashcards",     icon:"≡ƒâÅ", color:"#F59E0B", bg:"#FFFBEB", dashed:"#FCD34D", placeholder:"Write key points, definitions, rules to memorize..." },
      { textKey:"summary",   fileKey:"summaryFile", label:"Summary",        icon:"Γëí",  color:"#10B981", bg:"#F0FDF4", dashed:"#6EE7B7", placeholder:"Write a brief summary of this law / provision..." },
      { textKey:"examNotes", fileKey:"examFile",    label:"Exam Material",  icon:"≡ƒÄô", color:"#8B5CF6", bg:"#F5F3FF", dashed:"#C4B5FD", placeholder:"Write exam tips, past questions, key scenarios..." },
      ...
      <div key={textKey} style={{marginBottom:"1.25rem",background:bg,border:`1.5px solid ${dashed}`,borderRadius:"var(--radius)",padding:"1rem",transition:"box-shadow 0.15s"}}>
        ...
        {f[fileKey]?(
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"#fff",border:`1px solid ${dashed}`,borderRadius:"var(--radius-sm)"}}>
            <span style={{fontSize:18,flexShrink:0}}>≡ƒôä</span>
            <div style={{flex:1,minWidth:0}}>
            ...
            <a href={fileViewUrl(f[fileKey].name)} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()}
              style={{padding:"3px 9px",fontSize:11,background:"#fff",border:`1px solid ${dashed}`,borderRadius:20,color,textDecoration:"none",fontWeight:600,flexShrink:0}}>≡ƒæü View</a>
            <button onClick={()=>s(fileKey,null)}
              style={{padding:"3px 9px",fontSize:11,background:"#FEE2E2",border:"1px solid #FECACA",borderRadius:20,color:"#7F1D1D",cursor:"pointer",flexShrink:0}}>Γ£ò Remove</button>
          </div>
        ):(
          <FileUploadZone
...
/* ΓöÇΓöÇΓöÇ Detail Modal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
function DetailModal({item,onClose,onEdit,onDelete,onAI}){
  ...
  return(<Modal title={item.title} onClose={onClose} wide>
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16,alignItems:"center"}}>
      ...
      <button onClick={onAI} style={{marginLeft:"auto",background:"#FFF8EC",border:"1.5px solid #FCD34D",color:"#854F0B",padding:"5px 14px",borderRadius:20,fontWeight:600,fontSize:12,cursor:"pointer"}}>Γùê AI Actions</button>
    </div>
    ...
    {item.attachedFile&&(
      <div style={{marginTop:12,marginBottom:4,background:"#F0FDF4",border:"1.5px solid #86EFAC",borderRadius:"var(--radius-sm)",padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}>
        <span style={{fontSize:24,flexShrink:0}}>≡ƒôä</span>
        <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",gap:8,flexShrink:0}}>
          ...
          <button onClick={()=>downloadFile(item.attachedFile.name)} style={{padding:"6px 14px",background:"#1E40AF",color:"#fff",border:"none",borderRadius:"var(--radius-sm)",fontSize:12,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>Γ¼ç Download</button>
        </div>
      </div>
    )}
    ...
    {item.docTypeId==="foodforthought"&&item.fullText&&(
      <div style={{marginTop:14,background:"#FFF7ED",border:"1.5px solid #FED7AA",borderRadius:10,padding:"16px 18px"}}>
        <p style={{fontSize:10,fontWeight:800,color:"#C2410C",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10}}>≡ƒºá Answer / Analysis</p>
    ...
    {item.docTypeId==="foodforthought"&&item.articleNo&&(
      <div style={{marginTop:10,background:"#EFF6FF",border:"1.5px solid #BFDBFE",borderRadius:10,padding:"14px 16px"}}>
        <p style={{fontSize:10,fontWeight:800,color:"#1D4ED8",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>ΓÜû Law References</p>
    ...
    {item.docTypeId==="foodforthought"&&item.examNotes&&(
      <div style={{marginTop:10,background:"#F0FDF4",border:"1.5px solid #BBF7D0",borderRadius:10,padding:"14px 16px"}}>
        <p style={{fontSize:10,fontWeight:800,color:"#15803D",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Γ£à Key Takeaway</p>
        <p style={{fontSize:12.5,lineHeight:1.8,color:"#14532D",whiteSpace:"pre-wrap"}}>{item.examNotes}</p>
      </div>
    )}
    <DBlock l="Full Text"         v={item.fullText}/>
    <DBlock l="Full Text" v={item.fullText}/>
    {item.tags&&<p style={{fontSize:12,color:"var(--text2)",marginTop:12}}>≡ƒÅ╖ {item.tags}</p>}

    {/* Notes, Flashcards, Summary, Exam ΓÇö text + file */}
    <div style={{marginTop:16,display:"flex",flexDirection:"column",gap:12}}>
      {[
        {textKey:"notes",     fileKey:"notesFile",   label:"Notes",         icon:"Γ£Å",  color:"#0EA5E9", bg:"#F0F9FF", bd:"#7DD3FC"},
        {textKey:"flashcards",fileKey:"flashFile",   label:"Flashcards",    icon:"≡ƒâÅ", color:"#F59E0B", bg:"#FFFBEB", bd:"#FCD34D"},
        {textKey:"summary",   fileKey:"summaryFile", label:"Summary",       icon:"Γëí",  color:"#10B981", bg:"#F0FDF4", bd:"#6EE7B7"},
        {textKey:"examNotes", fileKey:"examFile",    label:"Exam Material", icon:"≡ƒÄô", color:"#8B5CF6", bg:"#F5F3FF", bd:"#C4B5FD"},
        ...
        <div key={textKey} style={{background:bg,border:`1.5px solid ${bd}`,borderRadius:"var(--radius-sm)",padding:"12px 14px"}}>
          ...
          {item[fileKey]&&(
            <a href={fileViewUrl(item[fileKey].name)} target="_blank" rel="noreferrer"
              ...
              ≡ƒôä {item[fileKey].name}{item[fileKey].pages>0?` (${item[fileKey].pages}p)`:""} ΓÇö Open PDF
            </a>
          )}
        </div>
      ))}
    ...
    {item.attachedFile&&(
      <div style={{marginTop:12,background:"#F0FDF4",border:"1.5px solid #6EE7B7",borderRadius:"var(--radius-sm)",padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <a href={fileViewUrl(item.attachedFile.name)} target="_blank" rel="noreferrer"
          style={{padding:"5px 12px",background:"#059669",border:"none",borderRadius:20,color:"#fff",fontWeight:600,fontSize:12,textDecoration:"none"}}>Open ΓåÆ</a>
      </div>
    )}
...
/* ΓöÇΓöÇΓöÇ Table Row ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function TableRow({item,onClick,onMove}){
  return(
    <tr onClick={onClick}
      ...
      <td style={{padding:"10px 12px",fontSize:12,color:"var(--text2)",whiteSpace:"nowrap"}}>{item.year||"ΓÇö"}</td>
      <td style={{padding:"10px 12px",fontSize:12,color:"var(--text2)",whiteSpace:"nowrap"}}>{item.month||"ΓÇö"}</td>
      <td style={{padding:"10px 12px",fontSize:12,color:"var(--text2)",whiteSpace:"nowrap",maxWidth:100,overflow:"hidden",textOverflow:"ellipsis"}}>{item.reference||item.decisionNo||item.publicationRef||"ΓÇö"}</td>
      ...
      <td style={{padding:"10px 12px",fontSize:12,color:"var(--text2)",maxWidth:180}}>
        <p style={{display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden",lineHeight:1.5}}>{item.notes||"ΓÇö"}</p>
      </td>
      <td style={{padding:"10px 12px",textAlign:"center"}}>
        {item.notesFile
          ?<a href={fileViewUrl(item.notesFile.name)} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{background:"#F0F9FF",color:"#0EA5E9",fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,textDecoration:"none"}}>≡ƒôä PDF</a>
          :<span style={{color:"var(--text3)",fontSize:12}}>ΓÇö</span>}
      </td>
      <td style={{padding:"10px 12px",textAlign:"center"}}>
        {item.flashFile
          ?<a href={fileViewUrl(item.flashFile.name)} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{background:"#FFFBEB",color:"#F59E0B",fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,textDecoration:"none"}}>≡ƒôä PDF</a>
          :<span style={{color:"var(--text3)",fontSize:12}}>ΓÇö</span>}
      </td>
      <td style={{padding:"10px 12px",textAlign:"center"}}>
        {item.summaryFile
          ?<a href={fileViewUrl(item.summaryFile.name)} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{background:"#F0FDF4",color:"#10B981",fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,textDecoration:"none"}}>≡ƒôä PDF</a>
          :<span style={{color:"var(--text3)",fontSize:12}}>ΓÇö</span>}
      </td>
      <td style={{padding:"10px 12px",textAlign:"center"}}>
        {item.examFile
          ?<a href={fileViewUrl(item.examFile.name)} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{background:"#F5F3FF",color:"#8B5CF6",fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,textDecoration:"none"}}>≡ƒôä PDF</a>
          :<span style={{color:"var(--text3)",fontSize:12}}>ΓÇö</span>}
      </td>
...
/* ΓöÇΓöÇΓöÇ Entry Card (search results only) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  return(<div onClick={onClick}
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}>
      <div style={{display:"flex",gap:6}}>
        {item.attachedFile&&<span style={{fontSize:11,color:"#059669",fontWeight:600}}>≡ƒôÄ Law PDF</span>}
        {item.summaryFile&&<span style={{fontSize:11,color:"#1D4ED8",fontWeight:600}}>≡ƒôä Summary</span>}
        {item.examFile&&<span style={{fontSize:11,color:"#7C3AED",fontWeight:600}}>≡ƒôä Exam</span>}
        {item.flashcards&&<span style={{fontSize:11,color:"#92400E",fontWeight:600}}>≡ƒâÅ Flashcards</span>}
      </div>
      <span style={{fontSize:11,color:"var(--text3)"}}>{fmt(item.updatedAt||item.createdAt)}</span>
    </div>
  </div>);
}
/* ΓöÇΓöÇΓöÇ Drop Destination Picker ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    <div
      ...
      {hovering&&!droppedData&&(
        <div style={{textAlign:"center",color:"#fff",pointerEvents:"none"}}>
          <p style={{fontSize:64,marginBottom:12}}>≡ƒôé</p>
      ...
      <div style={{background:"#fff",borderRadius:16,padding:"28px 32px",width:580,maxWidth:"95vw",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 25px 80px rgba(0,0,0,0.5)"}}>
        ...
        {!droppedData?(
          <>
            <h3 style={{fontSize:16,fontWeight:800,color:"#1E3A5F",marginBottom:12,textAlign:"center"}}>≡ƒöù Add from Link or File</h3>
            {/* Drop zone */}
            <div style={{border:"3px dashed #BFDBFE",borderRadius:12,padding:"28px 20px",textAlign:"center",background:"#F0F9FF",marginBottom:12}}>
              <p style={{fontSize:32,marginBottom:8}}>Γ¼ç</p>
            ...
            <p style={{fontSize:10,color:"#94A3B8",marginBottom:12,textAlign:"center"}}>
              Right-click any PDF link ΓåÆ "Copy link address" ΓåÆ paste above
            </p>
          ...
          <>
            ...
            <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:8,padding:"8px 14px",marginBottom:16,fontSize:12,color:"#065F46"}}>
              {droppedData.names.map((n,i)=><div key={i} style={{marginBottom:2}}>≡ƒôä {n}</div>)}
            </div>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>onSave(droppedData.files,droppedData.url,destCat,destDt)}
                ...
                Γ£ô Save to {selCat?.label} / {selDt?.label}
              </button>
...
/* ΓöÇΓöÇΓöÇ Category Page ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function CatPage({cat,entries,files=[],onAdd,onBulkSave,onItem,activeType,onChangeType,onSearchAll}){
  ...
  const buildRows=()=>{
    ...
    allTypeEntries.forEach(e=>{
      // Text field matches ΓåÆ one row per field
      textKeys.forEach(k=>{
        if(!e[k]) return;
        const val=e[k];
      ...
      // PDF matches ΓåÆ one row per page
  ...
  async function bulkUpload(fileList){
    ...
    for(let i=0; i<pdfs.length; i++){
      ...
      try{
        ...
        if(errMsg==="Failed to fetch"){
          setBulkStatus("Server connection error ΓÇö please refresh and try again.");
    ...
    // Background text extraction ΓÇö runs after entries appear, updates file-meta for search
    setBulkUploading(false);
    (async()=>{
      let extracted = 0;
      for(const entry of results){
        ...
        try{
          ...
          extracted++;
          setBulkStatus("Extracting text: "+extracted+"/"+results.length+" done ΓÇö search will find these now");
      ...
      setBulkStatus("Done! "+results.length+" entr"+(results.length!==1?"ies":"y")+" added. Text extracted for "+extracted+" PDFs ΓÇö search is ready.");
    ...
    <div>
      {showDropPicker&&<DropDestPicker cat={cat}
        ...
        onSave={async(droppedFiles,droppedUrl,destCatId,destDtId)=>{
          ...
          if(droppedFiles&&droppedFiles.length>0){
            ...
            setBulkStatus("Γ£ô "+results.length+" saved to "+TAX_CATS.find(c=>c.id===destCatId)?.label);
            ...
            try{
              ...
              if(res.ok&&res.files[0]){
                ...
                setBulkStatus("Γ£ô Saved to "+TAX_CATS.find(c=>c.id===destCatId)?.label+": "+cleanTitle);
      ...
      <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:200,position:"relative"}}>
          <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:14,color:"var(--text3)",pointerEvents:"none"}}>Γîò</span>
          <input value={q} onChange={e=>setQ(e.target.value)}
            placeholder={"Search in "+dt.label+" ΓÇö name, notes, inside PDFs..."}
            style={{...IS,paddingLeft:36,width:"100%"}}/>
          {q&&<button onClick={()=>setQ("")} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"var(--border)",border:"none",borderRadius:"50%",width:18,height:18,fontSize:12,cursor:"pointer",color:"var(--text2)",display:"flex",alignItems:"center",justifyContent:"center"}}>├ù</button>}
        </div>
        ...
        <button onClick={()=>!bulkUploading&&bulkRef.current?.click()}
          ...
          {bulkUploading?"ΓÅ│ Uploading...":"≡ƒôñ Bulk Upload PDFs"}
        </button>
        ...
        <button onClick={async()=>{
          ...
          for(const entry of noText){
            try{
              ...
              const text=r.text||"";
              if(!text) continue;
              const stored=text; // No limit ΓÇö full document stored
              ...
              setBulkStatus("Extracted "+done+"/"+noText.length+" ΓÇö "+entry.attachedFile.name.slice(0,30));
          ...
          ≡ƒöì Extract PDF Text {typeEntries.filter(e=>e.attachedFile&&!pdfText(e.attachedFile)).length>0&&"("+typeEntries.filter(e=>e.attachedFile&&!pdfText(e.attachedFile)).length+" pending)"}
        </button>
      </div>
      {bulkStatus&&<p style={{fontSize:12,color:bulkStatus.startsWith("Γ£ô")?"#059669":"#854F0B",marginBottom:8,fontWeight:600,padding:"6px 12px",background:bulkStatus.startsWith("Γ£ô")?"#F0FDF4":"#FFFBEB",borderRadius:"var(--radius-sm)",border:"1px solid "+(bulkStatus.startsWith("Γ£ô")?"#6EE7B7":"#FCD34D")}}>{bulkStatus}</p>}
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",padding:"8px 12px",background:"#F8FAFF",borderRadius:"var(--radius-sm)",border:"1px solid var(--border)",marginBottom:12}}>
        ...
        <select value={sortBy} onChange={e=>setSortBy(e.target.value)}
          ...
          <option value="year-desc">Date: Newest First (Dec 2025 ΓåÆ Jan 2020)</option>
          <option value="year-asc">Date: Oldest First (Jan 2020 ΓåÆ Dec 2025)</option>
          <option value="title-asc">Name: A ΓåÆ Z</option>
          <option value="title-desc">Name: Z ΓåÆ A</option>
                ...
                DOC_TYPES.filter(d=>c.id==="ct"||c.id==="vat"?true:!["cabinet","ministerial","fta"].includes(d.id)).map(d=>{
                  ...
                  return(
                    <button key={c.id+"-"+d.id} disabled={isCurrent}
                      ...
                      {c.label} ΓåÆ {d.label} {isCurrent?"(current)":""}
                    </button>
                  );
                })
              )}
      ...
      {searching&&rows.length===0&&(
        <div style={{textAlign:"center",padding:"3rem",background:"#fff",borderRadius:"var(--radius)",border:"1.5px solid var(--border)",color:"var(--text2)"}}>
          <p style={{fontSize:22,marginBottom:8}}>≡ƒöì</p>
      ...
      {/* SEARCH RESULTS ΓÇö flat row table */}
      {searching&&rows.length>0&&(
                  ...
                  <tr key={ri}
                    ...
                    <td style={{padding:"10px 12px",verticalAlign:"top",textAlign:"center"}}>
                      {row.inPdf&&row.page?(
                        ...
                        <span style={{fontSize:11,color:"var(--text3)"}}>ΓÇö</span>
                      )}
                    </td>
                  </tr>
                );
...
/* ΓöÇΓöÇΓöÇ AI Panel ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function AIPanel({item,file,onClose}){
  ...
  const ACTS=[{id:"summarize",label:"Summarize",icon:"Γëí"},{id:"quiz",label:"Generate Quiz",icon:"?"},{id:"keypoints",label:"Key Points",icon:"Γÿà"},{id:"explain",label:"Explain Simply",icon:"ΓùÄ"}];

  return(<Modal title={`AI ΓÇö ${file?.name||item?.title}`} onClose={onClose} wide>
    ...
    {!loading&&quiz&&!quiz.done&&q&&(<div>
      ...
      {sel&&<div style={{background:fb?"#D1FAE5":"#FEE2E2",border:`1.5px solid ${fb?"#6EE7B7":"#FCA5A5"}`,borderRadius:"var(--radius-sm)",padding:"12px 14px",marginBottom:12}}>
        <p style={{fontWeight:700,fontSize:13,color:fb?"#064E3B":"#7F1D1D",marginBottom:4}}>{fb?"Γ£ô Correct":"Γ£ù Incorrect ΓÇö "+q.ans}</p>
        <p style={{fontSize:12,lineHeight:1.6,color:fb?"#065F46":"#991B1B"}}>{q.exp}</p>
      </div>}
      {sel&&<Btn onClick={nextQ} variant="primary">Next ΓåÆ</Btn>}
    </div>)}
...
function InlineCite({cite,entries,files,onNavigate}){
  ...
  if(hasLink){
    return(
      <button onClick={handleClick} title={(page?'Jump to Article '+artNum+' p.'+page+' ΓÇö ':'')+((match?match.title:pdfFile?.name)||'')}
      ...
      <div style={{flex:1}}>
        ...
        {hasLink&&(
          <button onClick={()=>{if(match&&onNavigate)onNavigate(match);else if(matchFile)window.open(BASE_URL+'/api/file/'+encodeURIComponent(matchFile.name),'_blank');}}
            ...
            {match?'ΓåÆ Open in Knowledge Base':'ΓåÆ View PDF'}
          </button>
        )}
        ...
        if(line.startsWith('- ')||line.startsWith('* ')){
          return(
            <div key={i} style={{display:'flex',gap:8,margin:'3px 0 3px 8px'}}>
              <span style={{color:'#C4973B',fontWeight:700,flexShrink:0,lineHeight:1.7}}>ΓÇ║</span>
      ...
      {refs.length>0&&(
        <div style={{marginTop:18,padding:'14px 16px',background:'#F8FAFF',border:'2px solid #DBEAFE',borderRadius:10}}>
          <p style={{fontSize:10,fontWeight:800,color:'#1E3A5F',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:10}}>
            ≡ƒôÜ References ΓÇö Click to verify
          </p>
...
function AIChat({entries,files,onNavigate}){
  const [msgs,setMsgs]=useState([{role:'assistant',content:'Hello Hassan! Ask me anything about UAE tax law ΓÇö I will answer directly with specific law citations.'}]);
  ...
  async function send(){
    ...
    if(attach){
      ...
      displayContent='≡ƒôÄ '+attach.name+(text?'\n'+text:'');
      ...
      displayContent='≡ƒö¼ '+text+(relevant.length>0?' ('+relevant.length+' docs)':'');
    } else {
  ...
  const suggestions=['Can we claim VAT on invoices before registration?','Small Business Relief ΓÇö who qualifies?','Qualifying income for Free Zone persons?','Transfer pricing documentation requirements','De minimis threshold rules and consequences'];
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 80px)',maxWidth:900,margin:'0 auto',padding:'0 1rem'}}>
      <div style={{textAlign:'center',padding:'1rem 0 0.75rem',flexShrink:0}}>
        <h1 style={{fontSize:20,fontWeight:800,color:'#1E3A5F',marginBottom:3}}>Γùê AI Research Assistant</h1>
        ...
        <div style={{display:'inline-flex',background:'#F3F4F6',borderRadius:8,padding:3,gap:2}}>
          {[{id:'research',label:'≡ƒö¼ Research'},{id:'chat',label:'≡ƒÆ¼ Chat'}].map(m=>(
      ...
      <div style={{flex:1,overflowY:'auto',padding:'0.5rem 0',display:'flex',flexDirection:'column',gap:12}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start',gap:10,alignItems:'flex-start'}}>
            {m.role==='assistant'&&(
              <div style={{width:28,height:28,borderRadius:'50%',background:'#1E3A5F',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:12,flexShrink:0,marginTop:2}}>Γùê</div>
            )}
        ...
        {loading&&(
          <div style={{display:'flex',gap:10,alignItems:'center'}}>
            <div style={{width:28,height:28,borderRadius:'50%',background:'#1E3A5F',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:12}}>Γùê</div>
      ...
      {attach&&(
        <div style={{padding:'6px 12px',background:'#FFFBEB',border:'1px solid #FCD34D',borderRadius:6,fontSize:11,color:'#854F0B',marginBottom:4,display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          ≡ƒôÄ {attach.name}
          <button onClick={()=>setAttach(null)} style={{background:'none',border:'none',cursor:'pointer',color:'#854F0B',fontSize:14}}>├ù</button>
        </div>
      )}
      ...
      <div style={{display:'flex',gap:8,padding:'0.75rem 0',flexShrink:0,borderTop:'1.5px solid #E5E7EB'}}>
        ...
        <input value={input} onChange={e=>setInput(e.target.value)}
          ...
          placeholder={mode==='research'?'Ask anything ΓÇö AI searches your PDFs and cites laws...':'Ask about UAE tax...'}
        ...
        <button onClick={send} disabled={loading||(!input.trim()&&!attach)}
          ...
          {mode==='research'?'≡ƒö¼ Research':'Send'}
        </button>
      </div>
    </div>
  );
...
/* ΓöÇΓöÇΓöÇ File Library ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function FileLib({files,setFiles,entries,onAI}){
  ...
  async function doExtract(){
    ...
    if(!todo.length){setMsg("Γ£ô All PDFs already extracted!");setTimeout(()=>setMsg(""),4000);return;}
    setExtracting(true);
    let done=0,fail=0;
    ...
    const fr=await fetch(`${API}/files-meta`);
    if(fr.ok)setFiles(await fr.json());
    setMsg("Γ£ô Done! "+done+" extracted"+(fail?" ┬╖ "+fail+" failed":"")+". All content is now searchable.");
    setExtracting(false);
  ...
  async function upload(fileList){
    ...
    try{
      ...
      const fr=await fetch(`${API}/files-meta`);
      if(fr.ok)setFiles(await fr.json());
      setMsg("Γ£ô "+res.files.length+" file(s) uploaded");
  ...
  return(
    <div style={{padding:0}}>
      ...
      <div style={{background:"#1E3A5F",borderRadius:12,padding:"20px 24px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
        <div>
          <h1 style={{fontSize:20,fontWeight:800,color:"#fff",marginBottom:4}}>≡ƒôü File Library</h1>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={doExtract} disabled={extracting}
            ...
            {extracting?"ΓÅ│ Extracting...":("≡ƒöì Extract All PDFs"+(pending>0?" ("+pending+" new)":""))}
          </button>
      ...
      {msg&&<div style={{marginBottom:12,padding:"10px 16px",background:msg.startsWith("Γ£ô")?"#F0FDF4":"#FFFBEB",border:"1px solid "+(msg.startsWith("Γ£ô")?"#86EFAC":"#FCD34D"),borderRadius:8,fontSize:12,fontWeight:600,color:msg.startsWith("Γ£ô")?"#166534":"#92400E"}}>{msg}</div>}

      {/* ΓöÇΓöÇ Drop zone ΓöÇΓöÇ */}
      <div onClick={()=>!uploading&&inputRef.current?.click()}
        ...
        <p style={{fontSize:13,color:"#64748B",fontWeight:600,margin:0}}>≡ƒôé Drop PDFs here or click to upload</p>
      </div>

      {/* ΓöÇΓöÇ Files grid ΓöÇΓöÇ */}
      {safeFiles.length===0?(
        <div style={{textAlign:"center",padding:"3rem",background:"#fff",borderRadius:12,border:"1.5px solid #E2E8F0"}}>
          <p style={{fontSize:32,marginBottom:8}}>≡ƒôä</p>
              ...
              <div key={f.id||f.name} style={{background:"#fff",border:"1.5px solid #E2E8F0",borderRadius:10,padding:14,display:"flex",flexDirection:"column",gap:8}}>
                ...
                <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  <span style={{fontSize:10,fontWeight:700,color:extracted?"#059669":"#D97706"}}>
                    {extracted?"Γ£ô Searchable":"ΓÅ│ Not extracted"}
                  </span>
                </div>
                <div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:4}}>
                  <a href={fileViewUrl(f.name)} target="_blank" rel="noreferrer"
                    style={{padding:"4px 10px",background:"#F1F5F9",border:"1px solid #E2E8F0",borderRadius:6,fontSize:11,fontWeight:600,textDecoration:"none",color:"#374151"}}>≡ƒæü Open</a>
                  <button onClick={()=>downloadFile(f.name)}
                    style={{padding:"4px 10px",background:"#1E40AF",color:"#fff",border:"none",borderRadius:6,fontSize:11,fontWeight:600,cursor:"pointer"}}>Γ¼ç</button>
                  <button onClick={()=>onAI(null,f)}
                    style={{padding:"4px 10px",background:"#F5F3FF",color:"#7C3AED",border:"1px solid #DDD6FE",borderRadius:6,fontSize:11,fontWeight:600,cursor:"pointer"}}>Γùê AI</button>
                  <button onClick={()=>setViewFile(f)}
                    style={{padding:"4px 10px",background:"#F1F5F9",border:"1px solid #E2E8F0",borderRadius:6,fontSize:11,fontWeight:600,cursor:"pointer",color:"#374151"}}>Γ£Å</button>
                  <button onClick={()=>del(f)}
                    style={{padding:"4px 10px",background:"#FEF2F2",color:"#DC2626",border:"1px solid #FECACA",borderRadius:6,fontSize:11,fontWeight:600,cursor:"pointer"}}>≡ƒùæ</button>
                </div>
              </div>
            );
          })}
...
/* ΓöÇΓöÇΓöÇ Search ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function SearchPage({entries,files=[],onItem,onNavigate,onFileAI,initialQ=""}){
  ...
  return(<div>
    ...
    <div style={{position:"relative",marginBottom:16}}>
      <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:"var(--text3)",fontSize:18,pointerEvents:"none"}}>Γîò</span>
      ...
      {q&&<button onClick={()=>{setQ("");setFilterCat("all");setFilterType("all");}} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"var(--surface2)",border:"none",borderRadius:"50%",width:22,height:22,fontSize:15,color:"var(--text2)",cursor:"pointer"}}>├ù</button>}
    </div>
    ...
    {!qLow&&<div style={{textAlign:"center",padding:"4rem 2rem",background:"#fff",borderRadius:"var(--radius)",border:"1.5px solid var(--border)"}}>
      <p style={{fontSize:36,marginBottom:12}}>Γîò</p>
    ...
    {qLow&&rows.length===0&&<div style={{textAlign:"center",padding:"3rem",background:"#fff",borderRadius:"var(--radius)",border:"1.5px solid var(--border)"}}>
      <p style={{fontSize:24,marginBottom:8}}>≡ƒöì</p>
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:10,marginBottom:28}}>
      <div style={{flex:1,position:"relative"}}>
        <span style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",color:"var(--text3)",fontSize:16}}>Γîò</span>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:12}}>
      {[["Total",entries.length,"≡ƒôÜ"],["Files",files.length,"≡ƒôü"],["Laws",entries.filter(e=>e.docTypeId==="laws").length,"ΓÜû"],["Cabinet Dec.",entries.filter(e=>e.docTypeId==="cabinet").length,"≡ƒÅ¢"],["Ministerial",entries.filter(e=>e.docTypeId==="ministerial").length,"≡ƒô£"],["FTA Dec.",entries.filter(e=>e.docTypeId==="fta").length,"≡ƒÅó"],["HM Notes",entries.filter(e=>e.docTypeId==="hmnotes").length,"Γ£Å"],["Exam",entries.filter(e=>e.docTypeId==="hmexam").length,"≡ƒÄô"]].map(([l,c,i])=>(
...
/* ΓöÇΓöÇΓöÇ Main App ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0F2847 0%,#1E3A5F 50%,#0F2847 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#fff",borderRadius:16,padding:"40px 48px",width:"100%",maxWidth:400,boxShadow:"0 25px 80px rgba(0,0,0,0.4)"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{fontSize:48,marginBottom:12}}>Γùê</div>
...
// Downloads polling removed
  ...
  // ΓöÇΓöÇ Backup / Restore ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return(<div style={{display:"flex",minHeight:"100vh"}}>
    ...
    {/* ΓöÇΓöÇ Sidebar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
    <div style={{width:220,background:"var(--nav)",display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh",overflow:"auto"}}>
      ...
      <div style={{padding:"1.1rem 1rem",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:36,height:36,background:"var(--accent)",borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>ΓÜû</div>
      ...
      <div style={{padding:"0.5rem 0.5rem 0"}}>
        {[{id:"dashboard",label:"Dashboard",icon:"Γè₧"},{id:"search",label:"Search",icon:"Γîò"},{id:"alldocs",label:"All Documents",icon:"≡ƒùé"}].map(n=>(
        ...
        {TAX_CATS.map(cat=>{
          ...
          return(<div key={cat.id}>
            <button onClick={()=>setExpanded(p=>({...p,[cat.id]:!p[cat.id]}))} style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:7,background:isActiveCat&&!isOpen?"var(--nav-active)":"transparent",border:"none",cursor:"pointer",textAlign:"left",color:isActiveCat?"var(--accent)":"rgba(255,255,255,0.65)",fontWeight:isActiveCat?600:400,fontSize:13,marginBottom:1}}>
              ...
              <span style={{fontSize:11,color:"rgba(255,255,255,0.3)",transform:isOpen?"rotate(180deg)":"none",transition:"transform 0.2s",display:"inline-block"}}>Γû╛</span>
            </button>
      ...
      <div style={{padding:"0.4rem 0.5rem",borderTop:"1px solid rgba(255,255,255,0.07)"}}>
        ...
        {[{id:"files",label:"File Library",icon:"≡ƒôü"},{id:"ai",label:"AI Assistant",icon:"Γùê"}].map(n=>(
    ...
    {/* ΓöÇΓöÇ Main content ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
    ...
    {/* ΓöÇΓöÇ Modals ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
    {modal&&(
      <Modal title={`${modal.editing?"Edit":"Add"} ΓÇö ${TAX_CATS.find(c=>c.id===modal.catId)?.label} ΓÇ║ ${DOC_TYPES.find(d=>d.id===modal.docTypeId)?.label}`} onClose={()=>setModal(null)}>
